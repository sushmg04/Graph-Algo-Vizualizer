...
class Window:
    def __init__(self, root):
        ...
        self.root.attributes('-fullscreen',True)  # Increased window size
        ...
        self.MainLabel = Label(self.root, text='Algorithm Visualizer', bg="LightSkyBlue1", fg="blue4",
                               font=("calibri bold", 36))  # Larger font
        self.MainLabel.pack(pady=40)  # Increased padding
        ...
        self.AlgoTypeList.config(bg="pink", activebackground="hot pink", cursor="hand2", font=("calibri", 20))  # Larger font
        self.AlgoNameList = OptionMenu(self.root, self.AlgoNameVar, 'None')
        self.AlgoNameList.config(bg="pink", activebackground="hot pink", cursor="hand2", font=("calibri", 20))  # Larger font
        # label of the two dropdown menus
        self.AlgoTypeVar.set("Select Algorithm Type")
        self.AlgoNameVar.set("Select Algorithm")
        self.AlgoTypeList.pack(pady=10)  # Increased padding
        self.AlgoNameList.pack(pady=10)  # Increased padding

        # next button
        self.NextButton = Button(self.root, text="Next>", bg="pale green", activebackground="lime green",
                                 command=self.Run1, font=("calibri", 24))  # Larger font
        self.NextButton.pack(pady=60)  # Increased padding
        
    #     self.run_bfs_dfs_button = Button(self.root, text="Run BFS/DFS GUI", command=self.run_bfs_dfs_gui)
    #     self.run_bfs_dfs_button.pack(pady=10)

    # def run_bfs_dfs_gui(self):
    #     # Execute bfs_dfs_gui.py using subprocess
    #     subprocess.Popen(["python", "bfs_dfs_gui.py"])
        self.run_bfs_dfs_button = Button(self.root, text="Run BFS/DFS GUI", command=self.run_bfs_dfs_gui)
        self.run_bfs_dfs_button.pack(pady=10)

        self.run_bfs_dfs_button = Button(self.root, text="Run Algorithm Info", command=self.algorithm_info)
        self.run_bfs_dfs_button.pack(pady=10)
    ...
    def algorithm_info(self):
        # Execute algorithm_info.py using subprocess
        subprocess.Popen(["python", "algorithm_info.py"])
    ...
    def Run1(self):
        ...
        # if backtracking is selected
            ...
            if self.AlgoNameVar.get() == "Sudoku":
                # size of the window
                wx, wy = 1000, 800  # Increased window size
                ...
                frame = Frame(self.root, width=1000, height=800, bg="aquamarine")
                frame.grid_propagate(0)
                frame.pack()
                # slider for speed
                self.speed_var = Scale(frame, label="Select Speed Of Visualizer :", from_=1, to=100, orient=HORIZONTAL,
                                       length=600, bg="powder blue", troughcolor="SteelBlue2", relief="solid",
                                       cursor="hand2")
                self.speed_var.place(relx=0.1, rely=0.05)
                ...
                next = Button(frame, text="Next>", bg="pale green", activebackground="lime green",
                              command=self.Run2, font=("calibri", 20))  # Larger font
                next.place(relx=0.5, rely=0.7)
                # button for main menu
                back = Button(frame, text="<Back", bg="pale green", activebackground="lime green",
                              command=self.Back, font=("calibri", 20))  # Larger font
                back.place(relx=0.2, rely=0.7)
                self.root.mainloop()
            elif self.AlgoNameVar.get() == "N-Queens" or self.AlgoNameVar.get() == "Knight's Tour":
                # size of the window
                wx, wy = 1000, 850  # Increased window size
                # fit the window at the centre of the screen
                wxs, wys = self.root.winfo_screenwidth(), self.root.winfo_screenheight()
                WINDOW_X, WINDOW_Y = (wxs / 2) - (wx / 2), (wys / 2) - (wy / 2)
                self.root.geometry('%dx%d+%d+%d' % (wx, wy, WINDOW_X, WINDOW_Y))
                ...
                if self.AlgoNameVar.get() == "N-Queens":
                    ...
                    try:
                        ...
                        self.root.tk.call('wm', 'iconphoto', self.root._w, img)
                # create a new frame for the speed slider and buttons
                frame = Frame(self.root, width=1000, height=850, bg="aquamarine")
                frame.grid_propagate(0)
                frame.pack()
                # slider for size
                if self.AlgoNameVar.get() == "N-Queens":
                    ...
                    self.size_var = Scale(frame, label="Select Size of the Chessboard :", from_=5, to=25,
                                          ...
                                          length=600, bg="powder blue", troughcolor="SteelBlue2", relief="solid",
                                          cursor="hand2")
                self.size_var.place(relx=0.1, rely=0.05)
                self.size_var["highlightthickness"] = 0
                # slider for speed
                self.speed_var = Scale(frame, label="Select Speed Of Visualizer :", from_=1, to=100, orient=HORIZONTAL,
                                       length=600, bg="powder blue", troughcolor="SteelBlue2", relief="solid",
                                       cursor="hand2")
                self.speed_var.place(relx=0.1, rely=0.4)
                self.speed_var["highlightthickness"] = 0
                # button for starting visualizer
                next = Button(frame, text="Next>", bg="pale green", activebackground="lime green",
                              command=self.Run2, font=("calibri", 20))  # Larger font
                next.place(relx=0.5, rely=0.8)
                # button for main menu
                back = Button(frame, text="<Back", bg="pale green", activebackground="lime green",
                              command=self.Back, font=("calibri", 20))  # Larger font
                back.place(relx=0.2, rely=0.8)
                self.root.mainloop()
        elif self.AlgoTypeVar.get() == "Searching":
            # Destroy the main window
            self.root.destroy()
